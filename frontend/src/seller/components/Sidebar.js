...
const Icons = {
  ...
  ChevronLeft: () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>,
  ChevronRight: () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>,
...
const Sidebar = () => {
  const { t, language, switchLanguage, logout, sidebarOpen, toggleSidebar, seller } = useSeller();
  const location = useLocation();

  const menuItems = [
    { path: '/seller', icon: 'Dashboard', label: 'dashboard' },
    { path: '/seller/products', icon: 'Package', label: 'products' },
    { path: '/seller/orders', icon: 'ShoppingCart', label: 'orders' },
    { path: '/seller/finance', icon: 'DollarSign', label: 'finance' },
    { path: '/seller/inventory', icon: 'Inventory', label: 'inventory' },
    { path: '/seller/promotions', icon: 'Tag', label: 'promotions' },
  ...
  return (
    <aside className={`fixed top-0 ${language === 'ar' ? 'right-0' : 'left-0'} h-full bg-gradient-to-b from-emerald-900 to-emerald-950 text-white transition-all duration-300 z-50 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
      <div className="h-16 flex items-center justify-between px-4 border-b border-emerald-700">
        {sidebarOpen && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg">Ocean Seller</span>
          </div>
        )}
        <button onClick={toggleSidebar} className="p-2 hover:bg-emerald-800 rounded-lg transition-colors">
          {sidebarOpen ? (language === 'ar' ? <Icons.ChevronRight /> : <Icons.ChevronLeft />) : (language === 'ar' ? <Icons.ChevronLeft /> : <Icons.ChevronRight />)}
        </button>
      </div>
        ...
        <div className="p-4 border-b border-emerald-700">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center">
              <span className="font-semibold">{seller.name?.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <p className="font-medium text-sm">{seller.name}</p>
              <p className="text-xs text-emerald-300 capitalize">{language === 'ar' ? 'بائع' : 'Seller'}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="p-4 space-y-2">
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive(item.path) ? 'bg-emerald-500 text-white' : 'text-emerald-200 hover:bg-emerald-800 hover:text-white'}`}>
              <Icon />
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 space-y-2 border-t border-emerald-700">
        <button onClick={() => switchLanguage(language === 'ar' ? 'en' : 'ar')} className="w-full flex items-center gap-3 px-4 py-3 text-emerald-200 hover:bg-emerald-800 hover:text-white rounded-xl transition-all">
          <Icons.Globe />
          {sidebarOpen && <span>{language === 'ar' ? 'English' : 'عربي'}</span>}
        </button>
        <a href="/" className="flex items-center gap-3 px-4 py-3 text-emerald-200 hover:bg-emerald-800 hover:text-white rounded-xl transition-all">
          <Icons.Store />
